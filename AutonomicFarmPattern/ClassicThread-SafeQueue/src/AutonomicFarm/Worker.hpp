...
class Worker {
   ...
   public:
    ...
    Worker(int ID, std::function<T(U x)> fun, SafeQueue<Task<T, U> *> **inputQueue, SafeQueue<Task<T, U> *> *outputQueue, SafeQueue<Feedback *> *feedbackQueueWorker) {
        this->function = fun;
    ...
    void start() {
        // if (ID == 0) {
        //     std::cout << "Worker a " << &(*this->inputQueue) << std::endl;

        //     Task<T, U> *t = (*inputQueue)->safe_pop();
