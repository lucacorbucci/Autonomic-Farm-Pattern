...
class AutonomicFarmSQ {
   ...
   public:
    AutonomicFarmSQ(int nWorker, std::function<T(U x)> function, int tsGoal, std::vector<Task<T, U>*> inputVector, bool collector) {
    ...
    void start() {
        ...
        for (int i = 0; i < this->nWorker; i++) {
            this->workerQueue.push_back(new WorkerSQ<T, U>{i, this->function, inputQueues->at(i), this->outputQueue, this->feedbackQueueWorker, collector, this->nWorker, this->tsGoal});
        }

        EmitterSQ<T, U> e{this->inputQueues, workerQueue, nWorker, this->feedbackQueue, this->inputVector, this->feedbackQueueWorker};

        CollectorSQ<T, U> c{this->outputQueue, this->nWorker, this->tsGoal, this->feedbackQueue};
