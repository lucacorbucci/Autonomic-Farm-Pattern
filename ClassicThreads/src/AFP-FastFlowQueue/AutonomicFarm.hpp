...
class AutonomicFarm {
   ...
   public:
    ...
    void start() {
        ...
        for (int i = 0; i < this->nWorker; i++) {
            this->workerQueue.push_back(new Worker<T, U>{i, this->function, this->inputQueues[i], this->outputQueue, this->feedbackQueueWorker, collector, this->nWorker, this->tsGoal});
        }

        Emitter<T, U> e{this->inputQueues, workerQueue, nWorker, this->feedbackQueue, this->inputVector, this->feedbackQueueWorker, collector};
        Collector<T, U>* c;
        if (collector) {
            c = new Collector<T, U>(this->outputQueue, this->nWorker, this->tsGoal, this->feedbackQueue);
        }
