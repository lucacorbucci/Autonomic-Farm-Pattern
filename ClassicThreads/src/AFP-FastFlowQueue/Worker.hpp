...
class Worker {
        ...
        if (!inputQueue->empty()) {
            if (inputQueue->pop(&tmpTask)) {
                ...
                if (t->end == -1) {
                    ...
                    // Feedback f = sendFeedback(newNWorker);
   ...
   public:
    ...
    Worker(int ID, std::function<T(U x)> fun, SWSR_Ptr_Buffer *inputQueue, uMPMC_Ptr_Queue *outputQueue, uMPMC_Ptr_Queue *feedbackQueueWorker, bool collector, int activeWorkers, int tsGoal) {
        this->function = fun;
